...
func NewManyToOne(size int, alerter vk.Alerter) *ManyToOne {
	if alerter == nil {
		alerter = vk.AlertFunc(func(int) {})
	}
...
func (d *ManyToOne) Set(data vk.GenericDataType) {
	for {
		...
		if old != nil &&
			(*vk.bucket)(old) != nil &&
			(*vk.bucket)(old).seq > writeIndex-uint64(len(d.buffer)) {
...
func (d *ManyToOne) TryNext() (data vk.GenericDataType, ok bool) {
	// Read a value from the ring buffer based on the readIndex.
	idx := d.readIndex % uint64(len(d.buffer))
	result := (*vk.bucket)(atomic.SwapPointer(&d.buffer[idx], nil))
